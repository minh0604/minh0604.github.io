...
let volSlier = document.getElementsByClassName("vol")[0];
...
volSlier.addEventListener("input", (e) => {
...
// add function of mute vollume
mute.addEventListener("click", muteVolume);
// make the media mute when hit mute button and come back vollume when they hit again.
function muteVolume() {
    if(media.muted){
      media.muted = false;
      volSlier.value = media.volume;
      
    } else{
      media.muted = true;
      volSlier.value = 0;
      /* play.setAttribute('data-icon', '?'); */
      //because the dafont.com dont have the volume on button so I cant change the button font.
    }      
      
  }


// add function of full screen
...
//add function of restart 
restart.addEventListener("click",restartMusic);
function restartMusic(){
  media.currentTime = 0;
}

//add funtion of down video

down.addEventListener("click",downVideo);
function downVideo(){
